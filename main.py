...
class ErrorWindow(CTkToplevel):
    def __init__(self, parent, message):
        ...
        self.close_button = CTkButton(master=self, text="Close", corner_radius=32,
                                      fg_color="#ffb114", hover_color="#bb7d00", command=self.destroy)
...
class MetaEdit(CTk):
    def __init__(self):
        ...
        self.font_path = os.path.join(
            current_dir, "Montserrat-VariableFont_wght.ttf")
    ...
    def calculateMetaData(self):
        ...
        with ExifToolHelper(executable=self.exiftool_path) as ET:
            ...
            for i in metadata:
                for j, k in i.items():
                    ...
                    if catagory not in self.catagories and catagory not in self.excluded:
                        ...
                        textbox = CTkTextbox(master=self.tabview.tab(
                            catagory), width=600, height=260, fg_color="#0f0f0f")
            ...
            self.deleteButton = CTkButton(self, text="Delete MetaData", width=125, height=25,
                                          command=self.stripMetaData, fg_color="#D22B2B", hover_color="#A52A2A", bg_color="#0f0f0f")

            self.deleteButton.place(
                x=375-(self.deleteButton.cget("width")/2), y=455)
